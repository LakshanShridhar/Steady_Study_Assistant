...
client = OpenAI(api_key="your OpenAI API key") # Replace "your OpenAI API key" with your actual OpenAI API key in the code.
...
def openai_question_validation(question):
    ...
    # Call the OpenAI API to get a response (using GPT-4 now)
    response = client.chat.completions.create(model="gpt-4",  # Change from gpt-3.5-turbo to gpt-4
    messages=messages,
    max_tokens=100,
    temperature=0.7)
